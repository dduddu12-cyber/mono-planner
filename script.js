...
async function applyImportPayload(data, mode) {
  ...
  try {
    ...
    if (mode === 'overwrite') {
      ...
      localStorage.setItem(REVIEW_KEY, JSON.stringify(data.reviews || {}));
      localStorage.setItem(REC_KEY,    JSON.stringify(data.recordings || {}));
      ...
      if (data.reviews && typeof data.reviews === 'object') {
        ...
        if (changed) localStorage.setItem(REVIEW_KEY, JSON.stringify(existingRv));
      }

      if (data.recordings && typeof data.recordings === 'object') {
        const existingRec = loadRecordings();
        let changed = false;
        Object.entries(data.recordings).forEach(([id, rec]) => {
          if (!existingRec[id]) { existingRec[id] = rec; changed = true; }
        });
        if (changed) localStorage.setItem(REC_KEY, JSON.stringify(existingRec));
      }
...
async function performSearch(query) {
  ...
  results.forEach(r => {
    ...
    el.addEventListener('click', async () => {
      if (typeof REC !== 'undefined' && REC.isRecording) {
        showToast('녹음 중에는 이동할 수 없습니다. 정지 후 이동해주세요.');
        return;
      }
      ...
      if (typeof REC !== 'undefined' && REC.pageVisible)       hideRecordingPage();
