...
async function _renderMemoPanelView() {
  ...
  mpv.innerHTML = `
    ...
    <textarea id="mpv-text" placeholder="메모 내용을 입력하세요..." style="flex:1;min-height:120px;border:none;border-top:1px solid var(--border);padding:0.85rem 1rem;font-size:0.9rem;line-height:1.7;resize:none;background:var(--bg);font-family:inherit;color:var(--text)">${escHtml(memo.text || '')}</textarea>
    ${memo.hasDrawing ? `
...
async function _saveMemoPanel(silent = false) {
  ...
  if (textEl)  memo.text  = textEl.value;
  await saveMemo(memo);
...
function makeTodoEl(task, type, weekKey, date) {
  ...
  item.querySelector('.todo-main').addEventListener('click', () => {
    if (type === 'daily') {
      ...
      if (task.linkedMemoId && S.memos[task.linkedMemoId]) {
...
let _scratchRafId   = null;
let _scratchPoints  = [];   // 배치할 포인트 버퍼

function initScratchCanvas() {
  const el = document.getElementById('scratch-canvas');
  if (!el) return;
  const ctx = el.getContext('2d', { desynchronized: true });
  S.scratchCanvas.el  = el;
  S.scratchCanvas.ctx = ctx;
  S.scratchCanvas.dpr = Math.min(window.devicePixelRatio || 1, 2);

  resizeScratchCanvas();
  if (!el._scratchResizeObserver) {
    el._scratchResizeObserver = new ResizeObserver(() => {
      S.scratchCanvas._cachedRect = null;
      resizeScratchCanvas();
    });
    el._scratchResizeObserver.observe(el.parentElement);
  }
  if (!el._scratchEventsAttached) {
    el.addEventListener('pointerdown',   onScratchPointerDown,   { passive: false });
    el.addEventListener('pointermove',   onScratchPointerMove,   { passive: false });
    el.addEventListener('pointerup',     onScratchPointerUp,     { passive: false });
    el.addEventListener('pointercancel', onScratchPointerUp);
    el._scratchEventsAttached = true;
  }
  applyScratchDrawStyle();
}

function resizeScratchCanvas() {
  const { el, ctx, dpr } = S.scratchCanvas;
  if (!el) return;
  const wrap = el.parentElement;
  const rect = wrap.getBoundingClientRect();
  const w = Math.max(rect.width, 200);
  ...
  ctx.lineCap  = 'round';
  ctx.lineJoin = 'round';
  S.scratchCanvas._cachedRect = el.getBoundingClientRect();
  if (saved && saved !== 'data:,') {
    const img = new Image();
    img.onload = () => ctx.drawImage(img, 0, 0, w, h);
    img.src = saved;
...
function _scratchGetXY(e) {
  if (!S.scratchCanvas._cachedRect) {
    S.scratchCanvas._cachedRect = S.scratchCanvas.el.getBoundingClientRect();
  }
  const r = S.scratchCanvas._cachedRect;
  return { x: e.clientX - r.left, y: e.clientY - r.top };
}

function onScratchPointerDown(e) {
  e.preventDefault();
  S.scratchCanvas.el.setPointerCapture(e.pointerId);
  S.scratchCanvas.drawing = true;
  _scratchPoints = [];
  const { x, y } = _scratchGetXY(e);
  S.scratchCanvas.lastX = x;
  S.scratchCanvas.lastY = y;
  applyScratchDrawStyle();
  // 시작 점 찍기
  const { ctx, size } = S.scratchCanvas;
  ctx.beginPath();
  ctx.arc(x, y, size / 2, 0, Math.PI * 2);
  ctx.fill();
}

function onScratchPointerMove(e) {
  if (!S.scratchCanvas.drawing) return;
  e.preventDefault();
  const { x, y } = _scratchGetXY(e);
  _scratchPoints.push({ x, y });
  if (!_scratchRafId) {
    _scratchRafId = requestAnimationFrame(_flushScratchPoints);
  }
}

function _flushScratchPoints() {
  _scratchRafId = null;
  if (!_scratchPoints.length) return;
  const sc  = S.scratchCanvas;
  const ctx = sc.ctx;
  applyScratchDrawStyle();
  ctx.beginPath();
  ctx.moveTo(sc.lastX, sc.lastY);
  for (const pt of _scratchPoints) {
    // 중점 베지어 곡선으로 부드러운 선
    const mx = (sc.lastX + pt.x) / 2;
    const my = (sc.lastY + pt.y) / 2;
    ctx.quadraticCurveTo(sc.lastX, sc.lastY, mx, my);
    sc.lastX = pt.x;
    sc.lastY = pt.y;
  }
  ctx.lineTo(sc.lastX, sc.lastY);
  ctx.stroke();
  _scratchPoints = [];
}

function onScratchPointerUp() {
  if (_scratchRafId) {
    cancelAnimationFrame(_scratchRafId);
    _scratchRafId = null;
    _flushScratchPoints();
  }
  S.scratchCanvas.drawing = false;
}

function applyScratchDrawStyle() {
  const { ctx, tool, color, size } = S.scratchCanvas;
  if (tool === 'eraser') {
    ctx.globalCompositeOperation = 'destination-out';
    ctx.lineWidth   = size * 5;
    ctx.strokeStyle = 'rgba(0,0,0,1)';
...
async function saveScratchPad() {
  const { el } = S.scratchCanvas;
  if (!el) return;
  const titleInput = document.getElementById('scratch-save-title');
  const title = titleInput.value.trim() || `낙서 ${new Date().toLocaleString('ko-KR', { month:'numeric', day:'numeric', hour:'2-digit', minute:'2-digit' })}`;
  ...
  await saveMemo(memo);
  document.getElementById('scratch-save-bar').classList.add('hidden');
  renderScratchList();
  showToast(`"${title}" 저장되었습니다.`);
}

function renderScratchList() {
  const container = document.getElementById('scratch-list-items');
  if (!container) return;
  const saves = Object.values(S.memos)
    .filter(m => m.type === 'scratch')
    .sort((a, b) => b.updatedAt.localeCompare(a.updatedAt));

  if (saves.length === 0) {
    container.innerHTML = '<p class="scratch-list-empty">저장된 낙서가 없습니다.</p>';
    return;
  }
  container.innerHTML = '';
  saves.forEach(m => {
    const item = document.createElement('div');
    item.className = 'scratch-list-item';
    item.innerHTML = `
      <img class="scratch-thumb" src="${m.drawingData || ''}" alt="${escHtml(m.title)}">
      <div class="scratch-item-info">
        <div class="scratch-item-title">${escHtml(m.title)}</div>
        <div class="scratch-item-date">${fmtShort(m.updatedAt)}</div>
      </div>
      <div class="scratch-item-actions">
        <button class="btn-sm" data-id="${m.id}" data-action="load">불러오기</button>
        <button class="btn-sm btn-danger-sm" data-id="${m.id}" data-action="del">삭제</button>
      </div>
    `;
  ...
  container.querySelectorAll('button[data-action]').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      if (btn.dataset.action === 'del') {
        if (!confirm('이 낙서를 삭제할까요?')) return;
        await deleteMemoById(id);
        renderScratchList();
      } else if (btn.dataset.action === 'load') {
        const m = S.memos[id];
        if (!m || !m.drawingData) return;
        clearScratchCanvas();
        const { ctx, el, dpr } = S.scratchCanvas;
        const w = el.width / dpr;
        const h = el.height / dpr;
        const img = new Image();
        img.onload = () => ctx.drawImage(img, 0, 0, w, h);
        img.src = m.drawingData;
        showToast(`"${m.title}" 불러왔습니다.`);
      }
    });
  });
}
...
function bindEvents() {
  ...
  // 낙서장
  document.getElementById('w-scratch-widget').addEventListener('click', () => {
    showModal('scratch');
    setTimeout(() => { initScratchCanvas(); renderScratchList(); }, 50);
  });
  document.querySelector('[data-modal-close="scratch"]')?.addEventListener('click', () => hideModal('scratch'));
  document.getElementById('scratch-tool-pen').addEventListener('click', () => {
    S.scratchCanvas.tool = 'pen';
    document.getElementById('scratch-tool-pen').classList.add('active');
    document.getElementById('scratch-tool-eraser').classList.remove('active');
    document.getElementById('scratch-canvas').style.cursor = 'crosshair';
  });
  document.getElementById('scratch-tool-eraser').addEventListener('click', () => {
    S.scratchCanvas.tool = 'eraser';
    document.getElementById('scratch-tool-eraser').classList.add('active');
    document.getElementById('scratch-tool-pen').classList.remove('active');
    document.getElementById('scratch-canvas').style.cursor = 'cell';
  });
  document.getElementById('scratch-pen-color').addEventListener('change', e => {
    S.scratchCanvas.color = e.target.value;
  });
  document.getElementById('scratch-pen-size').addEventListener('change', e => {
    S.scratchCanvas.size = parseFloat(e.target.value);
  });
  document.getElementById('scratch-tool-clear').addEventListener('click', () => {
    if (confirm('낙서장을 전체 지울까요?')) clearScratchCanvas();
  });

  // 낙서 저장 버튼
  document.getElementById('btn-scratch-save').addEventListener('click', () => {
    document.getElementById('scratch-save-bar').classList.remove('hidden');
    document.getElementById('scratch-save-title').value = '';
    document.getElementById('scratch-save-title').focus();
  });
  document.getElementById('btn-scratch-save-cancel').addEventListener('click', () => {
    document.getElementById('scratch-save-bar').classList.add('hidden');
  });
  document.getElementById('btn-scratch-save-confirm').addEventListener('click', saveScratchPad);
  document.getElementById('scratch-save-title').addEventListener('keydown', e => {
    if (e.key === 'Enter') saveScratchPad();
    if (e.key === 'Escape') document.getElementById('scratch-save-bar').classList.add('hidden');
  });

  // 낙서 목록 토글
  document.getElementById('btn-scratch-list').addEventListener('click', () => {
    const panel = document.getElementById('scratch-list-panel');
    panel.classList.toggle('hidden');
    if (!panel.classList.contains('hidden')) renderScratchList();
  });
