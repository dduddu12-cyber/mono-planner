/* =============================================================
   ...
   Review (IndexedDB 'reviews' store, keyPath: id):
   { id, type, title, date, rating, shortReview, content,
     images: [ "data:image/jpeg;base64,...", ... ],  // 최대 3장
     with, location, officialRating, travelStart, travelEnd,
     brand, store, price, createdAt, updatedAt }
   ※ 구버전에는 localStorage("mono_reviews")에 저장했으나 브라우저별
     5MB 한도 문제로 IndexedDB로 이전함 (migrateReviewsFromLocalStorage)
   ─────────────────────────────────────────────────────────── */
...
/* ── 화면 줌 ────────────────────────────────────────────────── */
const ZOOM_KEY    = 'mp_zoom_idx';
const ZOOM_LEVELS = [0.7, 0.8, 0.9, 1.0, 1.1, 1.2, 1.35];
const ZOOM_LABELS = ['70%', '80%', '90%', '100%', '110%', '120%', '135%'];
let _zoomIdx = 3; // 기본값: 100%

function initZoom() {
  const saved = parseInt(localStorage.getItem(ZOOM_KEY), 10);
  if (!isNaN(saved) && saved >= 0 && saved < ZOOM_LEVELS.length) _zoomIdx = saved;
  applyZoom(false);
}

function applyZoom(save = true) {
  document.documentElement.style.zoom = ZOOM_LEVELS[_zoomIdx];
  if (save) localStorage.setItem(ZOOM_KEY, _zoomIdx);
  const lbl = document.getElementById('zoom-level');
  if (lbl) lbl.textContent = ZOOM_LABELS[_zoomIdx];
...
function zoomIn()  { if (_zoomIdx < ZOOM_LEVELS.length - 1) { _zoomIdx++; applyZoom(); } }
function zoomOut() { if (_zoomIdx > 0)                      { _zoomIdx--; applyZoom(); } }
...
function renderMonthlyView() {
  ...
  // 날짜별 할일 집계 (로드된 주차 기준)
  const CATEGORY_DEFAULT_COLOR = { '일정': '#3182ce', '약속': '#d69e2e', '개인': '#805ad5', '과제': '#38a169', '시험': '#cc3333', '연차': '#cc3333', '권장휴가': '#cc3333', '시간휴가': '#cc3333', '휴무': '#cc3333' };
  const CALENDAR_SHOW    = new Set(['일정', '약속', '개인', '과제', '시험', '연차', '권장휴가', '시간휴가', '휴무']);
  const VACATION_CATS    = new Set(['연차', '권장휴가', '휴무']);
  const schedules    = {};   // dateStr → [{ text, completed, color, category }, ...]
  const otherTasks   = {};   // dateStr → [{ completed, color }, ...]
  const vacationDates = new Set(); // 휴가/연차가 있는 날짜
  for (const wd of Object.values(S.weeks)) {
    for (const [dateStr, todos] of Object.entries(wd.dailyTodos || {})) {
      ...
      for (const t of active) {
        if (CALENDAR_SHOW.has(t.category)) {
          if (!schedules[dateStr]) schedules[dateStr] = [];
          const displayColor = t.color || CATEGORY_DEFAULT_COLOR[t.category];
          schedules[dateStr].push({ text: t.text, completed: t.completed, color: displayColor, category: t.category });
          if (VACATION_CATS.has(t.category)) vacationDates.add(dateStr);
        } else {
  ...
  const cellsHtml = cells.map(({ dateStr, inMonth, day, dowIdx }) => {
    ...
    if (isSat) cls += ' sat';
    if (isSun || holidayName || isVacation) cls += ' sun';
    ...
    if (schList.length > 0) {
      ...
      schHtml = showSch.map(s => {
        const colorStyle = s.completed
          ? (s.color ? ` style="border-left:3px solid ${s.color}"` : '')
          : (s.color ? ` style="background:${s.color}"` : '');
        const label = s.category ? `<span class="monthly-event-cat">[${escHtml(s.category)}]</span> ` : '';
        const fullText = (s.category ? `[${s.category}] ` : '') + s.text;
        return `<div class="monthly-event${s.completed ? ' done' : ''}"${colorStyle} title="${escHtml(fullText)}">${label}${escHtml(s.text)}</div>`;
      }).join('');
      if (schList.length > 3) {
...
async function saveWeek(weekData) {
  ...
  if (typeof GistSync !== 'undefined') GistSync.scheduleSync();
}
...
async function _renderMemoPanelView() {
  ...
  if (memo.html) {
    ...
    initHtml = hasHtml ? memo.text
      : memo.text.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\n/g,'<br>');
  } else if (Array.isArray(memo.boxes) && memo.boxes.length > 0) {
    const firstText = memo.boxes.find(b => b.type === 'text');
    if (firstText) initHtml = firstText.content || '';
  }

  mpv.innerHTML = `
    <div style="padding:0.4rem 1rem 0;display:flex;align-items:center;gap:0.5rem;flex-wrap:wrap;flex-shrink:0">
      <input id="mpv-title" type="text" value="${escHtml(memo.title||'')}" placeholder="메모 제목..."
             style="font-size:0.95rem;font-weight:600;border:none;border-bottom:1.5px solid transparent;border-radius:0;background:transparent;padding:0.2rem 0;flex:1;min-width:0;transition:border-color 0.15s">
      <span id="mpv-save-time" style="font-size:0.75rem;color:var(--text3)">${memo.updatedAt ? '저장: ' + fmtShort(memo.updatedAt) : ''}</span>
      <button id="mpv-back-btn" style="font-size:0.78rem;padding:0.2rem 0.6rem;border-radius:var(--radius);background:var(--bg3);color:var(--text2);border:1px solid var(--border);cursor:pointer">← 날짜 보기</button>
    </div>
    <div class="input-toolbar" id="mpv-toolbar" style="flex-shrink:0">
      <button type="button" class="tool-btn" id="mpv-btn-newline" title="줄바꿈 삽입">↵ 줄바꿈</button>
      <button type="button" class="tool-btn" id="mpv-btn-backspace" title="한 글자 지우기">⌫ 지우기</button>
      <button type="button" class="tool-btn" id="mpv-btn-space" title="스페이스 삽입">␣ 스페이스</button>
      <button type="button" class="tool-btn" id="mpv-btn-img" title="이미지 파일 삽입">📎 이미지</button>
      <input type="file" id="mpv-img-input" accept="image/*" style="display:none">
    ...
    <div id="mpv-text" class="daily-note-full" contenteditable="true" spellcheck="false" style="flex:1;overflow-y:auto" data-placeholder="내용을 입력하세요..."></div>
  `;

  // 내용 채우기
  const textEl = document.getElementById('mpv-text');
  textEl.innerHTML = initHtml;

  const mpvFocus = () => textEl.focus();
  ...
  // 툴바 버튼
  document.getElementById('mpv-btn-newline').addEventListener('mousedown', e => e.preventDefault());
  document.getElementById('mpv-btn-newline').addEventListener('click', () => { mpvFocus(); document.execCommand('insertLineBreak'); });
  document.getElementById('mpv-btn-backspace').addEventListener('mousedown', e => e.preventDefault());
  document.getElementById('mpv-btn-backspace').addEventListener('click', () => { mpvFocus(); document.execCommand('delete'); });
  document.getElementById('mpv-btn-space').addEventListener('mousedown', e => e.preventDefault());
  document.getElementById('mpv-btn-space').addEventListener('click', () => { mpvFocus(); document.execCommand('insertText', false, ' '); });

  // 이미지 삽입
  document.getElementById('mpv-btn-img').addEventListener('click', () => document.getElementById('mpv-img-input').click());
  document.getElementById('mpv-img-input').addEventListener('change', e => {
  ...
  // 글자색 / 형광펜
  mpv.querySelectorAll('.mpv-fmt-btn').forEach(btn => {
    btn.addEventListener('mousedown', e => e.preventDefault());
    btn.addEventListener('click', () => {
      mpvFocus();
      if (btn.dataset.action === 'color') document.execCommand('foreColor', false, btn.dataset.value);
      else if (btn.dataset.action === 'highlight') document.execCommand('hiliteColor', false, btn.dataset.value);
    });
  });

  // 자동 저장
  textEl.addEventListener('input', () => scheduleMpvSave());
...
async function saveMpvContent() {
  ...
  const textEl = document.getElementById('mpv-text');
  if (textEl) memo.html = textEl.innerHTML;
...
async function openExportModal() {
  ...
  const reviewCount    = Object.keys(await loadReviews()).length;
...
function openGistModal() {
  document.getElementById('gist-pat-input').value  = localStorage.getItem(GIST_PAT_KEY)  || '';
  document.getElementById('gist-id-input').value   = localStorage.getItem(GIST_ID_KEY)   || '';
  const lastSync = localStorage.getItem(GIST_SYNC_KEY);
  const info = document.getElementById('gist-status-info');
  if (info) {
    if (GistSync.isConfigured()) {
      info.style.display = 'block';
      info.innerHTML = `
        <div>동기화 상태: <b>${navigator.onLine ? '온라인' : '오프라인'}</b></div>
        <div>마지막 동기화: <b>${lastSync ? fmtLong(lastSync) : '없음'}</b></div>
        <div style="margin-top:0.4rem;color:var(--text3);font-size:0.8rem;">Gist ID: ${GistSync.gistId || '-'}</div>
      `;
    } else {
...
/* ── 가져오기 핵심 로직 (doImport + GistSync pullOnLoad 공통) ─────── */
async function applyImportPayload(data, mode) {
  if (typeof GistSync !== 'undefined') GistSync._inPull = true;
  try {
    const db = await openDB();
    ...
    if (mode === 'overwrite') {
      await new Promise((resolve, reject) => {
        ...
        tx.objectStore('weeks').clear().onsuccess = resolve;
        tx.onerror = reject;
      });
      for (const w of data.weeks) await dbPut('weeks', w);
      S.weeks = {};
      data.weeks.forEach(w => { S.weeks[w.weekKey] = w; });

      await new Promise((resolve, reject) => {
        const tx  = db.transaction('memos', 'readwrite');
        tx.objectStore('memos').clear().onsuccess = resolve;
        tx.onerror = reject;
      });
      S.memos = {};
      if (Array.isArray(data.memos)) {
        for (const m of data.memos) { await dbPut('memos', m); S.memos[m.id] = m; }
      }
      ...
      if (Array.isArray(data.notebooks)) {
        for (const nb of data.notebooks) { await dbPut('notebooks', nb); S.notebooks[nb.id] = nb; }
      }
      ...
      S.nbPages = {};
      if (Array.isArray(data.nb_pages)) {
        for (const p of data.nb_pages) { await dbPut('nb_pages', p); S.nbPages[p.id] = p; }
      }

      await saveReviews(data.reviews || {});
      ...
      if (Array.isArray(data.memos)) {
        for (const m of data.memos) {
          if (!S.memos[m.id] && !await dbGet('memos', m.id)) {
            await dbPut('memos', m); S.memos[m.id] = m;
          }
        }
      }

      if (data.reviews && typeof data.reviews === 'object') {
        const existingRv = await loadReviews();
        let changed = false;
        Object.entries(data.reviews).forEach(([id, rv]) => {
          if (!existingRv[id]) { existingRv[id] = rv; changed = true; }
        });
        if (changed) await saveReviews(existingRv);
      }
    ...
    if (typeof GistSync !== 'undefined') GistSync._inPull = false;
  }
}
...
async function doImport() {
  ...
  await applyImportPayload(S.importPayload, S.importMode);
  hideModal('import');
  S.importPayload = null;
  await loadCurrentWeek();
  renderAll();
  renderHistoryWeekNav();
  if (RV.reviewPageVisible) await renderReviewPage();
...
async function performSearch(query) {
  ...
  results.forEach(r => {
    ...
    el.addEventListener('click', async () => {
      ...
      closeSearch();
      if (typeof RV  !== 'undefined' && RV.reviewPageVisible)  hideReviewPage();
      if (typeof NL  !== 'undefined' && NL.pageVisible)        hideNotesPage();
      if (S.activeNbPageId) closeNbPage();
...
function bindEvents() {
  ...
  if (moreBtn && moreMenu) {
    ...
    const mmGist = document.getElementById('mm-gist-sync');
    if (mmGist) {
      mmGist.addEventListener('click', () => {
        moreMenu.classList.add('hidden');
        openGistModal();
      });
    }
  ...
  // Gist 동기화 버튼 / 모달
  document.getElementById('btn-gist-sync').addEventListener('click', openGistModal);
  const syncWidget = document.getElementById('w-sync-widget');
  if (syncWidget) syncWidget.addEventListener('click', openGistModal);
  document.getElementById('btn-gist-save').addEventListener('click', async () => {
    const pat    = document.getElementById('gist-pat-input').value.trim();
    const gistId = document.getElementById('gist-id-input').value.trim();
    if (!pat) { showToast('Personal Access Token을 입력하세요.'); return; }
    GistSync.saveConfig(pat, gistId);
    hideModal('gist');
    showToast('설정이 저장되었습니다. Gist에 동기화합니다...');
    await GistSync.push();
  });
  document.getElementById('btn-gist-pull').addEventListener('click', async () => {
    const pat    = document.getElementById('gist-pat-input').value.trim();
    const gistId = document.getElementById('gist-id-input').value.trim();
    if (!pat || !gistId) { showToast('PAT와 Gist ID를 먼저 입력하고 저장하세요.'); return; }
    GistSync.saveConfig(pat, gistId);
    hideModal('gist');
    await GistSync.manualPull();
    await loadCurrentWeek();
    renderAll();
    renderHistoryWeekNav();
  });
...
/* =====================================================================
   REVIEW MODULE  ── IndexedDB 'reviews' store (keyPath: id)
   구버전 호환: 예전에는 localStorage("mono_reviews")에 base64 이미지를
   통째로 저장해 브라우저별 5MB 한도에 걸렸음 → IndexedDB로 이전.
   ===================================================================== */

/* ── 리뷰 데이터 접근 ─────────────────────────────────────────────── */

const REVIEW_KEY = 'mono_reviews'; // 구버전 localStorage 키 (마이그레이션용)

// 최초 1회: localStorage에 남아있던 리뷰 데이터를 IndexedDB로 이전하고 비운다.
async function migrateReviewsFromLocalStorage() {
  const raw = localStorage.getItem(REVIEW_KEY);
  if (!raw) return;
  try {
    const reviews = JSON.parse(raw);
    for (const review of Object.values(reviews)) {
      if (review && review.id) await dbPut('reviews', review);
    }
  } catch { /* 손상된 데이터는 건너뜀 */ }
...
async function loadReviews() {
  const list = await dbGetAll('reviews');
  const map = {};
  list.forEach(r => { map[r.id] = r; });
  return map;
}

// 리뷰 맵 전체를 통째로 교체 (백업 복원 등 대량 쓰기용)
async function saveReviews(reviews) {
  ...
  await new Promise((resolve, reject) => {
    ...
    store.clear();
    Object.values(reviews).forEach(r => store.put(r));
...
async function openReviewModal(prefillDate, editId) {
...
async function saveReview() {
  ...
  const existing   = RV.editId ? (await dbGet('reviews', RV.editId)) || {} : {};
...
async function _injectDayCardReviewUI() {
  const reviews = await loadReviews();
  const byDate  = {};
...
async function _injectMonthlyReviewStars() {
  const reviews = await loadReviews();
...
async function rvDoImport() {
  ...
  if (S.importMode === 'overwrite') {
    ...
    if (S.importPayload.reviews && typeof S.importPayload.reviews === 'object') {
      await saveReviews(S.importPayload.reviews);
    }
    ...
    if (S.importPayload.reviews && typeof S.importPayload.reviews === 'object') {
      ...
      await saveReviews(Object.assign({}, S.importPayload.reviews, existing));
    }
  }
...
async function bindReviewEvents() {
  // ⓪ 구버전 localStorage 리뷰 데이터를 IndexedDB로 이전
...
function makeNotebookEl(nb) {
  ...
  pages.forEach((p, idx) => {
    ...
    item.innerHTML = `
      ...
      <div class="nb-page-order-btns">
        <button class="nb-page-order-btn" data-dir="up" title="위로"${idx === 0 ? ' disabled' : ''}>▲</button>
        <button class="nb-page-order-btn" data-dir="down" title="아래로"${idx === pages.length - 1 ? ' disabled' : ''}>▼</button>
      </div>
      <button class="nb-page-del-btn" title="삭제">✕</button>`;
    item.addEventListener('click', e => {
      if (e.target.closest('.nb-page-del-btn')) return;
      if (e.target.closest('.nb-page-order-btns')) return;
      openNbPage(p.id);
    });
    ...
    item.querySelector('.nb-page-order-btns').addEventListener('click', e => {
      e.stopPropagation();
      const btn = e.target.closest('.nb-page-order-btn');
      if (!btn || btn.disabled) return;
      reorderNbPage(p.id, btn.dataset.dir);
    });
...
async function reorderNbPage(pageId, dir) {
  const page = S.nbPages[pageId];
  if (!page) return;
  const sorted = Object.values(S.nbPages)
    .filter(p => p.notebookId === page.notebookId)
    .sort((a, b) => (a.sortOrder || 0) - (b.sortOrder || 0));
  ...
  a.sortOrder = b.sortOrder ?? swapIdx;
  b.sortOrder = tmpOrder;
  await Promise.all([saveNbPage(a), saveNbPage(b)]);
  renderNotebookTree();
}
...
function nbSetAllCollapsed(collapse) {
  const page = _cvCtx.page;
  if (!page || !page.boxes) return;
  page.boxes.forEach(box => {
    ...
    if (!el) return;
    el.classList.toggle('nb-box-collapsed', collapse);
    const btn = el.querySelector('.nb-box-collapse-btn');
    if (btn) { btn.textContent = collapse ? '▸' : '▾'; btn.title = collapse ? '펼치기' : '접기'; }
  });
  _cvCtx.saveFn();
}
...
function bindNotebookEvents() {
  ...
  // 모두 접기 / 모두 펼치기
  document.getElementById('btn-nb-collapse-all')?.addEventListener('click', () => nbSetAllCollapsed(true));
  document.getElementById('btn-nb-expand-all')?.addEventListener('click', () => nbSetAllCollapsed(false));
