...
async function _renderMemoPanelView() {
  ...
  mpv.innerHTML = `
    ...
    <div style="flex-shrink:0;border-top:1px solid var(--border);padding:0.3rem 0.75rem;display:flex;gap:0.3rem;align-items:center;flex-wrap:wrap;background:var(--bg2)">
      ...
      <button class="tool-btn" id="mpv-space" title="스페이스 삽입">␣ 스페이스</button>
      <button class="tool-btn" id="mpv-img-btn" title="이미지 파일 삽입">📎 이미지</button>
    ...
    <div id="mpv-text" contenteditable="true" spellcheck="false" style="flex:1;min-height:120px;border:none;border-top:1px solid var(--border);padding:0.85rem 1rem;font-size:18px;line-height:1.7;background:var(--bg);font-family:inherit;color:var(--text);outline:none;overflow-y:auto;word-break:break-word"></div>
    ${memo.hasDrawing ? `
  ...
  // 이미지 붙여넣기 (Ctrl+V)
  document.getElementById('mpv-text').addEventListener('paste', e => {
      ...
      if (item.type.startsWith('image/')) {
        ...
        reader.onload = ev => {
          ...
          if (sel && sel.rangeCount) {
            ...
            document.getElementById('mpv-text').appendChild(img);
          }
          debounceNote('memo-panel-' + _panelMemoId, () => _saveMemoPanel(true), 800);
        };
  ...
  document.getElementById('mpv-space').addEventListener('mousedown', e => e.preventDefault());
  document.getElementById('mpv-space').addEventListener('click', () => {
    document.getElementById('mpv-text').focus();
    document.execCommand('insertText', false, ' ');
  });

  // 이미지 파일 삽입 버튼
  document.getElementById('mpv-img-btn').addEventListener('click', () => {
    document.getElementById('mpv-img-input').click();
  });
...
const RV = {
  filterType: 'all',          // 'all' | 'restaurant' | 'book' | 'movie'
  editId:     null,           // 편집 중인 리뷰 id, null = 신규
  starRating: 0,              // 현재 별점 (0.5 단위)
  imageData:  null,           // 첨부 이미지 base64
...
function rvTypeName(type) {
  return type === 'restaurant' ? '🍽 식당' : type === 'book' ? '📖 책' : '🎬 영화';
}
...
function _rvSetImagePreview(dataUrl) {
  RV.imageData = dataUrl || null;
  const preview  = document.getElementById('rv-img-preview');
  const clearBtn = document.getElementById('rv-img-clear');
  const labelEl  = document.getElementById('rv-img-label');
  if (dataUrl) {
    preview.innerHTML = `<img src="${dataUrl}" alt="첨부 이미지">`;
    clearBtn.classList.remove('hidden');
    labelEl.textContent = '사진 변경';
  } else {
    preview.innerHTML  = '';
    clearBtn.classList.add('hidden');
    labelEl.textContent = '+ 사진 선택';
    document.getElementById('rv-img-input').value = '';
  }
}
...
function renderReviewPage() {
  ...
  list.forEach(r => {
    ...
    card.innerHTML = `
      ${r.imageData ? `<img class="rv-card-thumb" src="${r.imageData}" alt="${escHtml(r.title)}">` : ''}
      <div class="rv-card-top">
        <span class="rv-badge rv-badge-${r.type}">${rvTypeName(r.type)}</span>
        <span class="rv-card-title" title="${escHtml(r.title)}">${escHtml(r.title)}</span>
      </div>
      <div class="rv-card-meta">
        <span>${escHtml(r.date || '')}</span>
        <span class="rv-card-stars" title="${r.rating}점">${rvStarsFullHtml(r.rating)}</span>
        <span>${r.rating > 0 ? r.rating : ''}</span>
      </div>
...
function openReviewModal(prefillDate, editId) {
  ...
  _rvSetImagePreview(null);

  rvSetType('restaurant');
  ...
  if (editId) {
    ...
    if (r) {
      ...
      if (r.imageData) _rvSetImagePreview(r.imageData);
    }
  }
...
function rvSetType(type) {
  ...
  document.getElementById('rv-restaurant-fields').style.display =
...
async function saveReview() {
  ...
  const r = {
    ...
    with:        type === 'restaurant'
      ...
      : [],
    location:    type === 'restaurant' ? document.getElementById('rv-location-input').value.trim() : '',
    officialRating: type === 'restaurant' && document.getElementById('rv-official-rating-input').value !== ''
      ? parseFloat(document.getElementById('rv-official-rating-input').value) : null,
    imageData:   RV.imageData || null,
...
function _injectMonthlyReviewStars() {
  ...
  const TYPE_COLOR = { restaurant: '#dd6b20', book: '#3182ce', movie: '#805ad5' };
  const MAX_SHOW   = 2;
  ...
  document.querySelectorAll('#view-monthly .monthly-cell[data-date]').forEach(cell => {
    ...
    // .monthly-dots 또는 .monthly-week-hint 앞에 삽입
...
function bindReviewEvents() {
  ...
  // 기존 리스너 제거를 위해 cloneNode로 교체
  const newExportBtn = exportConfirmBtn.cloneNode(true);
  exportConfirmBtn.parentNode.replaceChild(newExportBtn, exportConfirmBtn);
  newExportBtn.addEventListener('click', rvDoExport);

  const newImportBtn = importConfirmBtn.cloneNode(true);
  importConfirmBtn.parentNode.replaceChild(newImportBtn, importConfirmBtn);
  newImportBtn.addEventListener('click', rvDoImport);
  ...
  document.getElementById('rv-img-input').addEventListener('change', async e => {
    ...
    try {
      ...
      _rvSetImagePreview(dataUrl);
    } catch {
      showToast('이미지를 불러올 수 없습니다.');
    }
  });
  document.getElementById('rv-img-clear').addEventListener('click', () => _rvSetImagePreview(null));
...
async function renderNotesPage() {
  ...
  if (NL.filterType !== 'daily') {
    allMemos.forEach(m => {
      const textPreview = (m.text || '').replace(/\s+/g, ' ').trim();
      if (!m.title && !textPreview && !m.hasDrawing) return; // 완전히 빈 메모 제외
      items.push({
        kind:       'memo',
        id:         m.id,
        date:       m.date || '',
...
async function recStartRecording() {
  try {
    ...
    REC.mediaRecorder.onstop = () => {
      ...
      document.getElementById('rec-audio-wrap').classList.remove('hidden');
...
function bindRecordingEvents() {
  ...
  document.getElementById('btn-recording-tab').addEventListener('click', () => {
    if (REC.pageVisible) hideRecordingPage(); else showRecordingPage();
  });
  ...
  /* 백업/복원 버튼을 recordings 포함 버전으로 교체 */
  const exportBtn = document.getElementById('btn-export-confirm');
  const importBtn = document.getElementById('btn-import-confirm');
  if (exportBtn) {
    const newE = exportBtn.cloneNode(true);
    exportBtn.parentNode.replaceChild(newE, exportBtn);
    newE.addEventListener('click', recDoExport);
  }
  if (importBtn) {
    const newI = importBtn.cloneNode(true);
    importBtn.parentNode.replaceChild(newI, importBtn);
    newI.addEventListener('click', recDoImport);
  }
}
