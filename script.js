...
function renderMonthlyView() {
  ...
  // 날짜별 할일 집계 (로드된 주차 기준)
  const pending   = {};   // dateStr → count (일정 제외)
  const done      = {};   // dateStr → count (일정 제외)
  const schedules = {};   // dateStr → [{ text, completed }, ...]
    ...
    for (const [dateStr, todos] of Object.entries(wd.dailyTodos || {})) {
      ...
      for (const t of active) {
        if (t.category === '일정') {
          ...
          schedules[dateStr].push({ text: t.text, completed: t.completed });
        } else {
          if (!t.completed) pending[dateStr] = (pending[dateStr] || 0) + 1;
          else              done[dateStr]    = (done[dateStr]    || 0) + 1;
        }
      }
    }
  }
  ...
  const cellsHtml = cells.map(({ dateStr, inMonth, day, dowIdx }) => {
    ...
    const p = pending[dateStr] || 0;
    const d = done[dateStr]    || 0;
    const total = p + d;
    ...
    if (schList.length > 0) {
      ...
      schHtml = showSch.map(s =>
        `<div class="monthly-event${s.completed ? ' done' : ''}" title="${escHtml(s.text)}">${escHtml(s.text)}</div>`
      ).join('');
      if (schList.length > 3) {
    ...
    if (total > 0) {
      ...
      for (let i = 0; i < show; i++) {
        dots += `<span class="monthly-dot ${i < p ? 'pending' : 'done'}"></span>`;
      }
...
async function switchView(mode) {
  ...
  document.getElementById('filter-bar').classList.toggle('hidden', isMonthly);
...
function applyFilters(todos) {
  ...
  if (S.filters.status === 'complete')   list = list.filter(t => t.completed);
  if (S.filters.status === 'incomplete') list = list.filter(t => !t.completed);
  if (S.filters.priority !== 'all')      list = list.filter(t => t.priority === S.filters.priority);
  if (S.filters.tags.length > 0) {
    list = list.filter(t => S.filters.tags.every(tag => t.tags.includes(tag)));
  }
  if (S.filters.query) {
  ...
  if (S.filters.sortCompleteBottom) {
    list.sort((a, b) => {
...
function collectTags(weekKey) {
  const wd   = getWeek(weekKey);
  const tags = new Set();
  wd.weeklyTodos.forEach(t => t.tags.forEach(tag => tags.add(tag)));
  Object.values(wd.dailyTodos).forEach(arr =>
    arr.forEach(t => t.tags.forEach(tag => tags.add(tag)))
  );
  return [...tags].sort();
}
...
function calcStats(weekKey) {
  ...
  let dyTotal = 0, dyIncomplete = 0;
  Object.values(wd.dailyTodos).forEach(arr => {
    const active = arr.filter(t => !t.deletedAt);
    dyTotal     += active.length;
    dyIncomplete += active.filter(t => !t.completed).length;
  });
  ...
  return { todayCount, wkIncomplete, dyIncomplete, pct, coCount };
}
...
function renderDashboard() {
  const { todayCount, wkIncomplete, dyIncomplete, pct, coCount } = calcStats(S.weekKey);
  document.getElementById('w-today-num').textContent    = todayCount;
  document.getElementById('w-progress-num').textContent  = pct + '%';
  document.getElementById('w-progress-fill').style.width = pct + '%';
  document.getElementById('w-wk-num').textContent   = wkIncomplete;
  document.getElementById('w-dy-num').textContent   = dyIncomplete;
  ...
  document.getElementById('w-restore-time').textContent = fmtShort(S.meta.lastRestoreAt);
}

function renderFilterTags() {
  const tags  = collectTags(S.weekKey);
  const group = document.getElementById('tag-filter-group');
  const btns  = document.getElementById('tag-filter-btns');
  if (tags.length === 0) { group.style.display = 'none'; return; }
  group.style.display = 'flex';
  btns.innerHTML = tags.map(tag => `
    <button class="filter-btn tag-filter-btn ${S.filters.tags.includes(tag) ? 'active' : ''}"
            data-tag="${escHtml(tag)}">${escHtml(tag)}</button>
  `).join('');
  btns.querySelectorAll('.tag-filter-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      const tag = btn.dataset.tag;
      if (S.filters.tags.includes(tag)) {
        S.filters.tags = S.filters.tags.filter(t => t !== tag);
      } else {
        S.filters.tags.push(tag);
      }
      renderAll();
    });
  });
...
function renderDetailPanel() {
  ...
  if (!S.selectedDate) {
    heading.textContent  = '메모 / 필기';
    noteEl.disabled      = true;
    noteEl.value         = '';
    noteTime.textContent = '';
    overlay.style.display = 'flex';
    overlay.innerHTML    = '<span style="line-height:2">일간 일정이나 날짜를 선택하면<br>여기에서 메모와 필기를 볼 수 있습니다.</span>';
    clearCanvas();
  ...
  _renderTaskMemoSection();
  loadDrawing(S.weekKey, S.selectedDate);
}
...
function openDailyTaskModal(weekKey, date, taskId) {
  ...
  const includeDrawChk  = document.getElementById('dt-include-drawing');
  const memoOptionsDiv  = document.getElementById('dt-memo-options');
  ...
  const memoTitleInput  = document.getElementById('dt-memo-title');

  createMemoChk.checked   = true;
  includeDrawChk.checked  = false;
  memoOptionsDiv.style.display  = 'block';
  ...
  if (!isNew && task && task.linkedMemoId && S.memos[task.linkedMemoId]) {
    ...
    existingInfo.innerHTML = `
      <div style="flex:1;min-width:0;">
        ...
        <div style="font-size:0.75rem;color:var(--text3)">필기 노트${m.hasDrawing ? ' · 필기 포함' : ''}</div>
      </div>
...
async function onDailyTaskSave(openMemoAfter = false) {
  ...
  const includeDrawing = document.getElementById('dt-include-drawing').checked;
  const memoTitleRaw   = document.getElementById('dt-memo-title').value.trim();
  ...
  if (!isNew) {
    ...
    task = { ...src, text, category, requester,
             linkedWeeklyTodoId: linkId || null,
             updatedAt: new Date().toISOString() };
  } else {
    task = makeTask({ text, category, requester, linkedWeeklyTodoId: linkId || null });
  }
  ...
  if (createMemo && (isNew || !linkedMemoId)) {
    const memo = makeLinkedMemo({
      ...
      type:         'note',
      title:        memoTitleRaw || text,
      hasDrawing:   includeDrawing,
      weekKey,
      date
    });
    await saveMemo(memo);
...
function bindEvents() {
  ...
  document.getElementById('dt-text-input').addEventListener('keydown', e => {
    if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); onDailyTaskSave(false); }
  });
  ...
  document.querySelectorAll('.filter-btn[data-filter]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      btn.classList.add('active');
      S.filters[filterType] = btn.dataset.val;
      renderAll();
    });
  });

  // 완료 하단 정렬 토글
  document.getElementById('btn-sort-complete').addEventListener('click', (e) => {
    S.filters.sortCompleteBottom = !S.filters.sortCompleteBottom;
    e.currentTarget.textContent = `완료 하단: ${S.filters.sortCompleteBottom ? 'ON' : 'OFF'}`;
    e.currentTarget.classList.toggle('active', S.filters.sortCompleteBottom);
    renderAll();
  });
  ...
  document.getElementById('tool-eraser').addEventListener('click', () => {
    S.canvas.tool = 'eraser';
    document.getElementById('tool-eraser').classList.add('active');
    document.getElementById('tool-pen').classList.remove('active');
    document.getElementById('drawing-canvas').style.cursor = 'cell';
  });
  document.getElementById('pen-color').addEventListener('change', e => { S.canvas.color = e.target.value; });
  document.getElementById('pen-size').addEventListener('change', e => { S.canvas.size = parseFloat(e.target.value); });
  document.getElementById('tool-clear').addEventListener('click', () => {
    showConfirm('전체 지우기', '이 날짜의 필기를 모두 지울까요?').then(ok => {
      if (!ok) return;
      clearCanvas();
      if (S.selectedDate) saveDrawing();
    });
  ...
  document.getElementById('w-wk-widget').addEventListener('click', () => {
    document.querySelectorAll('.filter-btn[data-filter="status"]').forEach(b => b.classList.remove('active'));
    document.querySelector('.filter-btn[data-filter="status"][data-val="incomplete"]').classList.add('active');
    S.filters.status = 'incomplete'; S.filters.priority = 'all';
    renderAll();
    if (window.innerWidth <= 900) switchMobileTab('weekly');
  });
  document.getElementById('w-dy-widget').addEventListener('click', () => {
    document.querySelectorAll('.filter-btn[data-filter="status"]').forEach(b => b.classList.remove('active'));
    document.querySelector('.filter-btn[data-filter="status"][data-val="incomplete"]').classList.add('active');
    S.filters.status = 'incomplete'; S.filters.priority = 'all';
    renderAll();
    if (window.innerWidth <= 900) switchMobileTab('daily');
  });
